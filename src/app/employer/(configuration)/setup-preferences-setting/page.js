...
const Page = () => {
  ...
  const [userData, setUserData] = useState([
    { id: "", question: "", value_type: "", options: [] },
  ]);
  ...
  const getData = async () => {
    try {
      const response = await axios.post(
        `${BaseAPI}/admin/employer/setup-preferences-setting`,
      ...
      const preferences = response.data.response?.existing_preferences || [];
      setUserData(
  ...
  const handleAddMore = () => {
    setUserData([...userData, { question: "", value_type: "", options: [] }]);
  };
  ...
  const handleClick = async () => {
    ...
    try {
      const confirmationResult = await Swal.fire({
        title: "Update?",
        text: "Do you want to update preferences setting?",
        icon: "question",
      ...
      if (confirmationResult.isConfirmed) {
        ...
        if (response.data.status === 200) {
          Swal.fire({
            title: "Preferences setting updated successfully!",
            icon: "success",
      ...
      Swal.fire({
        title: "Failed",
        text: "Could not update preferences settings",
        icon: "error",
        confirmButtonText: "Close",
      });
    }
  ...
  const removePreference = async (preferenceId) => {
    try {
      const response = await axios.post(
        `${BaseAPI}/admin/employer/remove-preference-section`,
      ...
      if (response.data.status === 200) {
        Swal.fire({
          title: "Preferences setting removed successfully!",
          icon: "success",
          confirmButtonText: "Close",
        });
      ...
      Swal.fire({
        title: "Failed",
        text: "Could not remove preferences settings",
        icon: "error",
        confirmButtonText: "Close",
      });
    }
          ...
          <div className="breadCumb1">
            <div className="flex gap-3 items-center">
              ...
              <div className="flex gap-2 items-center">
                ...
                <span>Setup Preferences Setting</span>
              </div>
            </div>
          </div>
          <div className="generalSetting">
            <div className="profilelogo">
              <h2>Setup Preferences Setting</h2>
            </div>
            {userData.map((data, index) => (
              <div className="dynamic-fields" key={index}>
                <div className="row">
                  <div className="col-lg-2 labelText">Question</div>
