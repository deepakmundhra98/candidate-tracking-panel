  ...
  const getData = async () => {
    try {
      ...
      const response = await axios.post(
        ...
        { user_type: "staff" }, // Pass null as the request body if not required
        {
          headers: {
...

    {
      ...
      renderCell: (params) => (
        <div className="process__actionnn" style={{ margin: "10px 0px" }}>
          ...
          {params.row.employer_id !== 0 && userAccess[4]?.Edit === 1 && (
            <Link
              href={`/staff/qualificationlist/edit/${params.row.slug}`}
                  ...
                  <div className="flex flex-col md:flex-row justify-between items-center w-full md:gap-4">
                    ...
                    {userAccess[4]?.Add === 1 && (
                      <div className="add__interviewwww">
                        <Link
                          href="/staff/qualificationlist/add"
